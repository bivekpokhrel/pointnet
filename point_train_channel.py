...
config_default = {
    ...
    'output_folder': '/Users/bivekpokhrel/PycharmProjects/database/data/new_z_trans',
...
sweep_config = {
    ...
    'parameters': {
        ...
        'num_epoch': {
            'value': 8 # number of epoch to run after resume epoch
        }
    }
}


sweep_id= wandb.sweep(sweep_config,project="Pointnet_training_allchannel8")
#
...
def build_set(csv_path,padding):
    ...
    train_data, val_test_data, train_labels, val_test_labels = train_test_split(inputs, labels, test_size=0.2) # shuffles only at the strat
...
#
def build_optimizer(network,learning_rate,option_opt):
    if option_opt == 'sgd':
...
def train(config=None):
    with wandb.init(config=config):
        ...
        optimizer = build_optimizer(model, learning_rate=config.learning_rate,option_opt=config.option_opt)
        ...
        train_loader = DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=0, collate_fn=collate_fn) # suffles in every batch and therefore every epoch with in the trainset
